<?php
/**
 * API: Cerrar barrera manualmente desde la app
 * Método: POST
...
if ($stmt_update->execute()) {
    ...
    // Registrar evento de cierre manual
    registrarEvento($conn, null, $id_usuario, 'CIERRE_MANUAL', 'PERMITIDO', 'APP', 'Cierre manual desde aplicación móvil');
    
    $conn->close();
    
    respuestaExito([
        'estado_barrera' => 'CERRADA',
        'usuario' => $usuario['nombre'] . ' ' . $usuario['apellido'],
        'fecha_hora' => date('Y-m-d H:i:s')
    ], 'Barrera cerrada exitosamente');
} else {
    $stmt_update->close();
    $conn->close();
    respuestaError('Error al cerrar la barrera', 500);
}
?>